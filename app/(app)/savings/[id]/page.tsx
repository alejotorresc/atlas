...
export default async function SavingsGoalDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {goal.target_date && <p className="text-sm text-[var(--ds-neutral-600)]">Fecha objetivo: {formatDateGT(goal.target_date)}</p>}

      <GoalDetailActions goal={goal} accounts={accounts.filter((a) => !a.is_archived)} />

      <div>
        <h2 className="mb-2 text-sm font-medium text-[var(--ds-neutral-500)]">Historial de aportes y retiros</h2>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {contributions.length === 0 && (
                <tr>
                  <td colSpan={3} className="px-4 py-6 text-center text-[var(--ds-neutral-500)]">
                    Sin movimientos todavia.
                  </td>
                </tr>
              )}
              {contributions.map((c) => (
                <tr key={c.id} className="border-t border-[var(--ds-neutral-100)]">
